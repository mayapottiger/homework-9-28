...
everything <- everything %>%
  ...
  select(GEOID, NAME.x, unemployment_rate, poverty_rate, white_percentage, total_pills)
...
ggplot(everything) +
  geom_point(aes(white_percentage, total_pills.x)) +
  labs(title="Total Pills and White Popluation", caption = "Source: DEA ARCOS database, via Washington Post", fill="") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(white_percentage, total_pills.x), method = "lm", se = FALSE)
...
everything <- everything %>%
  ...
  select(GEOID, NAME.x.x, unemployment_rate, white_percentage, poverty_rate, total_pills.x, estimate)
...
pills_median <- everything %>%
  inner_join(pills_population, by=c("NAME.x.x" = "NAME.x")) %>%
  select(GEOID, NAME.x.x, pills_per_person, total_pills, estimate)
...
pills_race <- everything %>%
  ...
  select(GEOID, NAME.x.x, black, white, asian, total_pills.x)
