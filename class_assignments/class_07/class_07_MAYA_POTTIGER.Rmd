...
ggplot(logan_county_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="Logan County Saw A Steady Flow Of Opioid Pills", subtitle = "Total Pills Stayed Above 6 Million From 2006-2012", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

# From 2006-2012, Logan County had a varying amount of opioid pills shiped with no real trend. For the entire time period, the total pills in the county stayed above 6 million. The highest total pills were in 2008, 2009 and 2011 ,and the lowest year was 2006. 
...
ggplot(WV_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="Kanawha County Had Highest Pill Shipments In West Virginia", subtitle = "Logan, Cabell, Mingo, Raleigh and Wood counties also had high numbers of pills", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
  facet_wrap(nrow=2, . ~ buyer_county) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Again, there isn't a steady trend of pill shipments across the six counties. Kanawha mostly had a steady increase, while Mingo peaked in 2008 and then decreased. Raleigh and Mingo mostly had an upward trend in pills, while Logan and Cabell mostly plateaued.
...
ggplot(md_2012_household_pills) +
  geom_point(aes(pills_per_person, estimate)) +
  labs(x="Pills Per Person", y="Median Income", title="In MD, Lower Income Counties Have Higher Pills Per Person", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(pills_per_person, estimate), method = "lm", se = FALSE) +
  geom_text_repel(aes(pills_per_person, estimate, label=buyer_county))
...
```{r}
country_population_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(year=="2012") %>%
  select(county_name, population, countyfips)
...
```{r}
country_2012 <- country_pills_2012 %>%
  inner_join(country_population_2012, by=("countyfips")) %>%
  select(buyer_county, dosage_unit, population, countyfips)
...
country_2012_pills_per_person <- country_2012 %>%
  select(buyer_county, dosage_unit, population, countyfips) %>%
  mutate(pills_per_person = (dosage_unit / population))
...
county_median_household_income <- county_median_household_income %>%
  mutate(countyfips = GEOID)
...
country_2012_household_pills <- county_median_household_income %>%
  inner_join(country_2012_pills_per_person, by=("countyfips"))

```

```{r}
ggplot(country_2012_household_pills) +
  geom_point(aes(pills_per_person, estimate)) +
  labs(x="Pills Per Person", y="Median Income", title="HED", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(pills_per_person, estimate), method = "lm", se = FALSE)
