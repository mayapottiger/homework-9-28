...
# I loaded each of the data sets and stored them as objects.

addresses <- read_csv("data/buyer_addresses.csv")
annual_year <- read_csv("data/buyer_annual_by_year.csv")
totals <- read_csv("data/buyer_totals.csv")
state_populations <- read_csv("data/state_population_per_year.csv")
...
# There are more records in by_year because there are more entries, whereas totals is grouping by the buyer_dea_no to get the total number of pills from all six years in the data set.

# Buyer_addresses is higher than buyer_totals because a pharmacy can have multiple locations, meaning it has more entries. The totals sorts by dea_no, so they are grouped more tightly.
...
# The Veterans Affairs Consolidated Mail Outpatient Pharmacy, located at 3725 Rivers Avenue in North Charleston, South Carolina had the most pills sent to it in total. According to an article from the Post and Courier, this pharmacy sends prescriptions to patients across the country who are enrolled in Veterans Affairs health care. 
...
practitioner_year <- annual_year %>%
  inner_join(addresses, by="buyer_dea_no")
...
practitioner_year %>%
  filter(buyer_state.x == "MD", year == "2010", buyer_bus_act.x == "PRACTITIONER") %>%
  arrange(desc(dosage_unit))

# Rakesh Mathur had the most total pills in Maryland in 2010 with 337,450 pills. This was more than 100,000 more pills than the next highest practitioner. According to the Maryland State Physician Board, Mathur was suspended in 2012 for five years and given a fine of $25,000. However, as of 2015, her suspensions was terminated, but she was still on probation. To ensure this was the same doctor, you would need their license number, which can also be entered into the physician board.
...
totals %>%
  inner_join(state_populations, by="buyer_state") %>%
...
# West Virginia had the most pills per person sent to it from 2006-2012.
...
totals_addresses <- totals %>%
  left_join(addresses, by="buyer_dea_no")

```

```{r}

totals_addresses <- addresses %>%
  left_join(totals, by="buyer_dea_no") %>%
...
totals_addresses <- totals %>%
  anti_join(addresses, by= "buyer_dea_no")

# I think it returns zero fields because there aren't any values that don't match.
...
final_question <- addresses %>%
  inner_join(totals, by="buyer_dea_no")

# This returned 148,831 observations and 14 variables.

```

```{r}

final_question_2 <- addresses %>%
  left_join(totals, by="buyer_dea_no")

# This returned 278,715 observations and 14 variables. This means that there are roughly 130,000 more addresses listed that are not matched between the tables. I figured this out by doing an inner join to see how many observations they had in common. Then, I did a left join to see how many entries were left out.
