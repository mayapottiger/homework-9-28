...
# The combination with the higest number of pills sent to Mingo County was Actavis Pharma, Inc -> Miami-Luken -> Tug Valley Pharmacy, Inc. with a total of 4,954,200 pills.
...
# These are not the same company. They have different DEA numbers, cities and addresses.
...
# I think there has to be a way to group like terms. As we can see in the spreadsheet above, many of the REPORTER_NAME entries are similar. There must be some sort of group function that allows you to say something about summing or consolidating the entries for "Cardinal Health" and "Amerisourcebergen" and "H. D. Smith Wholesale." Some function that adds the appropriate columns (DOSAGE_UNIT) and consolidates the others that are already the same and don't need to be mathematically joined (REPORTER_CITY).
